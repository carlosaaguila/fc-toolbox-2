...
randi_pathway = '/mnt/leif/littlab/users/aguilac/Projects/FC_toolbox/results/mat_output_v2/randi_spikes/randi_%s.mat';
filename_randi = sprintf(randi_pathway,fname{1});
...
%generate pathways for values, channel labels, and fs
values_pathway = '/mnt/leif/littlab/users/aguilac/Projects/FC_toolbox/results/mat_output_v2/values/values_%s.mat';
filename_values= sprintf(values_pathway,fname{1});

chlabels_pathway = '/mnt/leif/littlab/users/aguilac/Projects/FC_toolbox/results/mat_output_v2/chlabels/chlabels_%s.mat';
filename_chlabels= sprintf(chlabels_pathway,fname{1});

fs_pathway = '/mnt/leif/littlab/users/aguilac/Projects/FC_toolbox/results/mat_output_v2/fs/fs_%s.mat';
filename_fs = sprintf(fs_pathway,fname{1});

%GET DATA and check for existing loads
...
if exist(filename_values) ~= 0
    ...
    if length(values_all) ~= 1000
        disp('picking up unfinished file... proceeding to finish patient');
        for J = length(values_all):1000
            f_all = select_spikes(:,4);
            sp_time_all = select_spikes(:,5);
            data = download_ieeg_data(fname{f_all(J)},login_name,pwfile,[sp_time_all(J)-2,sp_time_all(J)+2],1);
            disp(J);
            disp('of');
            disp('1000');
            ...
            % SAVE restrictions at the halfway point
            savepoints = [500,1000];
            if J == savepoints(1)
                save(filename_values, 'values_all','-v7.3');
                save(filename_chlabels, 'ch_labels_all');
                save(filename_fs_all, 'ch_labels_all');
                disp('saved ALL at halfway mark');
        ...
        disp('file is already done.... moving on to next patient');
    end
    
else
    disp('starting a new patient')
    for J = 1:length(select_spikes)
        f_all = select_spikes(:,4);
        sp_time_all = select_spikes(:,5);
        data = download_ieeg_data(fname{f_all(J)},login_name,pwfile,[sp_time_all(J)-2,sp_time_all(J)+2],1);
        disp(J);
